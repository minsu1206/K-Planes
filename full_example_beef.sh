...
# CUDA_VISIBLE_DEVICES=1 PYTHONPATH='.' python plenoxels/main.py --device 0 --config-path ${config_path} expname=$expname num_steps=1 data_downsample=4 selection=[[1,3,6,9],[0]] # cut roasted beef

# # train
# PYTHONPATH='.' python plenoxels/main.py --device 0 --config-path ${config_path} expname=$expname data_downsample=2 selection=[[1,3,6,9],[0]]
...
# # eval
PYTHONPATH='.' python plenoxels/main.py --device 0 --config-path ${config_path} --validate-only --log-dir logs/sparse4/$expname expname=$expname num_steps=1 data_downsample=2
# # spiral path render
...
# # arc path render
PYTHONPATH='.' python plenoxels/main.py --device 0 --config-path ${config_path} --render-only-arc --log-dir logs/sparse4/$expname expname=$expname num_steps=1 data_downsample=2
